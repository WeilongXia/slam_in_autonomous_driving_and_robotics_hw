...
template <typename C, typename D, typename Getter> void ComputeMedian(const C &data, D &median, Getter &&getter)
{
    size_t len = data.size();
    assert(len > 1);
    std::vector<D> d;
...
template <typename S> bool FitLine2D(const std::vector<Eigen::Matrix<S, 2, 1>> &data, Eigen::Matrix<S, 3, 1> &coeffs)
{
    if (data.size() < 2)
    {
        return false;
...
template <typename T> inline float GetPixelValue(const cv::Mat &img, float x, float y)
{
    // boundary check
    if (x < 0)
        x = 0;
...
template <typename S, int rows, int cols> bool CheckNaN(const Eigen::Matrix<S, rows, cols> &m)
{
...
/// PCD点转Eigen
template <typename PointT, typename S = double, int N> inline Eigen::Matrix<S, N, 1> ToEigen(const PointT &pt)
{
...
template <typename T> inline Eigen::Matrix<T, 3, 3> SKEW_SYM_MATRIX(const Eigen::Matrix<T, 3, 1> &v)
{
...
template <typename T> inline Eigen::Matrix<T, 3, 3> SKEW_SYM_MATRIX(const T &v1, const T &v2, const T &v3)
{
    Eigen::Matrix<T, 3, 3> m;
...
template <typename T> Eigen::Matrix<T, 3, 3> RpyToRotM2(const T r, const T p, const T y)
{
...
template <typename S> inline Eigen::Matrix<S, 3, 1> VecFromArray(const std::vector<S> &v)
{
    return Eigen::Matrix<S, 3, 1>(v[0], v[1], v[2]);
}

template <typename S> inline Eigen::Matrix<S, 3, 3> MatFromArray(const std::vector<S> &v)
{
 ...
 * @return
 */
template <typename T, typename T2> void limit_in_range(T &&num, T2 &&min_limit, T2 &&max_limit)
{
    if (num < min_limit)
    {
        num = min_limit;
 ...
 */
template <class scalar> inline std::pair<scalar, scalar> cos_sinc_sqrt(const scalar &x2)
{
    using std::cos;
    using std::sin;
    using std::sqrt;
